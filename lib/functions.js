...
async function msgUpsert({ type, messages }, sock) {
  ...
  for (const m of messages) {
    ...
    msg.sender = await jidNormalizedUser(effectiveSender);

    const [senderUser, senderServer] = msg.sender.split('@');
    if (senderServer === 'lid') {
      const lidMap = sock.signalRepository?.lidMapping;
      const mapped = lidMap?.getPNForLID?.(senderUser);
      if (mapped) {
        msg.sender = await jidNormalizedUser(`${mapped}@s.whatsapp.net`);
      }
    }
    ...
    msg.sudo = sudoNumbers.includes(senderNum) || msg.sender === await jidNormalizedUser(sock.user.id);
      ...
      for (const plugin of getPlugins()) {
        ...
        if (isAllowed && sendMsg) {
          ...
          try {
            ...
            await sock.sendMessage(await jidNormalizedUser(sock.user.id), {
